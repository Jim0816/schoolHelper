...
<html>
  <head>
	...
	<link rel="stylesheet" type="text/css" href="css/market/showAllGoods.css ">
    ...
    <script type="text/javascript">
		...
		function goPage(){
		    var allPage = document.getElementById("allPage").value.replace(/\s/g, "");
		    var goPageNum = document.getElementById("goPageNum").value.replace(/\s/g, "");
		    var one = document.getElementById("one").value.replace(/\s/g, "");
		    var two = document.getElementById("two").value.replace(/\s/g, "");
		    if(goPageNum=='' || goPageNum=='0' || goPageNum==0){
		        //不跳转
		    }else{
		        //跳转
		        if(goPageNum<allPage+1){
		            location.href="${pageContext.request.contextPath }/market/1/10?one="+one+"&two="+two+"&currentPage="+goPageNum;
		        }
		    }
		}
    </script>
         ...
         <div class="bg_top_top" >
             ...
             <c:choose>
                 <c:when test="${sessionScope.user==null}">
		             ...
		             <a class="a1" href="${pageContext.request.contextPath }/person/user/login?pageSource=a2" style="float:left;margin-left:10px;font-size:80%;line-height:28px;text-decoration:none;">请登录</a> 
		             <a class="a1" href="${pageContext.request.contextPath }/person/user/regist?pageSource=a2" style="float:left;margin-left:20px;font-size:80%;line-height:28px;text-decoration:none;">[免费注册]</a>
                 </c:when>
                 <c:otherwise>
                     <a class="a1" href="${pageContext.request.contextPath }/person/1/1?pageSource=a2">
                     ...
                     <a class="a1" href="${pageContext.request.contextPath }/person/user/exit?pageSource=a2" style="float:left;margin-left:10px;font-size:80%;line-height:27px;text-decoration:none;">注销</a>
                 </c:otherwise>
             </c:choose>
             ...
             <a class="a1" href="${pageContext.request.contextPath }/person/1/6?pageSource=a2" style="float:right;margin-right:10px;font-size:80%;line-height:28px;text-decoration:none;">卖家中心</a>
             <a class="a1" href="${pageContext.request.contextPath }/person/1/1?pageSource=a2" style="float:right;margin-right:10px;font-size:80%;line-height:28px;text-decoration:none;">个人中心</a>
        </div>
        <div class="bg" onmouseover="javascript:document.getElementById('nevigationBar').style.visibility='hidden';document.getElementById('showBar').style.background='none'">
          <div class="head">
              ...
              <div class="head_2" onmouseover="hiddenClassifyDiv()">
                 ...
                 <div class="car" onmouseover="enterCar()" onmouseout="leaveCar()" onclick="javascript:location.href='${pageContext.request.contextPath }/person/1/3?pageSource=a2' ">
		            <div class="carRoad">
                  ...
                  <div class="center_2_bar">
                      <span class="pageSelect">
                          <c:if test="${sessionScope.searchResult.getTotalPages()>1}">
                              <c:choose>
					               <c:when test="${sessionScope.searchResult.getTotalPages()==sessionScope.searchResult.getCurrentPage() || sessionScope.searchResult.getTotalPages()==0}">
					               ...
					               <c:otherwise>
					                   <a href="${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=${sessionScope.searchResult.getCurrentPage()+1}" title="下一页"><span class="pageSelect_next" onmouseover="this.style.background='gray'" onmouseout="this.style.background='white'">&gt</span></a>
					               </c:otherwise>
					           </c:choose>
					           
					           <c:choose>
					               <c:when test="${sessionScope.searchResult.getCurrentPage()>1}">
					                   <a href="${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=${sessionScope.searchResult.getCurrentPage()-1}" title="上一页"><span class="pageSelect_pre" onmouseover="this.style.background='gray'" onmouseout="this.style.background='white'">&lt</span></a>
					               </c:when>
					               <c:otherwise>
			                      ...
			                      <span class="goods" onmouseover="this.style.borderColor='black'" onmouseout="this.style.borderColor='gray'">
			                          ...
			                          <span class="goods_info">
			                              ...
			                              <span class="goods_shop">
			                                 ...
			                                 <span style="float:left;margin-left:5px;line-height:30px;"><a href="${pageContext.request.contextPath }/market/1/12?direct=1&shopId=${item.get('shopId')}">${item.get('shopName')}</a></span>
			                              </span>
			                          </span>
			                      </span>
		                      </c:forEach>     
	              ...
	              <div class="center_3">
	                  <div style="float:right;margin-top:10px;margin-right:20px;">
	                     <c:choose>
			               ...
			               <c:otherwise>
			                   <span style="float:left;line-height:30px;width:78px;height:32px;font-size:90%;font-weight:500;background:#EEEEEE;border:1px solid gray;cursor:pointer;" onclick="javascript:location.href='${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=1'">首页</span>
			               </c:otherwise>
				         </c:choose>
				         
	                     <c:choose>
			               <c:when test="${sessionScope.searchResult.getCurrentPage()>1}">
			                   <span style="float:left;margin-left:10px;line-height:30px;width:78px;height:32px;font-size:90%;font-weight:500;background:#EEEEEE;border:1px solid gray;cursor:pointer;" onclick="javascript:location.href='${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=${sessionScope.searchResult.getCurrentPage()-1}'"><font style="color:gray;">&lt</font>上一页</span>
			               </c:when>
			               <c:otherwise>
	                     ...
	                     <c:choose>
			               ...
			               <c:otherwise>
			                   <span style="float:left;margin-left:10px;line-height:30px;width:78px;height:32px;font-size:90%;font-weight:500;background:#EEEEEE;border:1px solid gray;cursor:pointer;" onclick="javascript:location.href='${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=${sessionScope.searchResult.getCurrentPage()+1}'"><font style="color:gray;">下一页<font style="color:gray;">&gt</font></span>
			               </c:otherwise>
			            </c:choose>
					           
	                    <c:choose>
			               <c:when test="${sessionScope.searchResult.getCurrentPage()==sessionScope.searchResult.getTotalPages()}">
			               ...
			               <c:otherwise>
			                   <span style="float:left;margin-left:10px;line-height:30px;width:78px;height:32px;font-size:90%;font-weight:500;background:#EEEEEE;border:1px solid gray;cursor:pointer;" onclick="javascript:location.href='${pageContext.request.contextPath }/market/1/10?one=${sessionScope.one}&two=${sessionScope.two}&currentPage=${sessionScope.searchResult.getTotalPages()}'">尾页</span>
			               </c:otherwise>
				       </c:choose> 
	                     
	                     
	                  
	                     <span style="float:left;margin-left:20px;line-height:30px;height:32px;font-size:90%;font-weight:500;background:white;">共<font style="font-weight:600;">${sessionScope.searchResult.getTotalPages()}</font>页</span>
	                     <span style="float:left;margin-left:20px;line-height:30px;height:32px;font-size:90%;font-weight:500;background:white;">
		                     ...
		                     <input id="allPage" type="hidden" value="${sessionScope.searchResult.getTotalPages()}"/>
		                     <input id="one" type="hidden" value="${sessionScope.one}"/>
		                     <input id="two" type="hidden" value="${sessionScope.two}"/>
		                     <input id="goPageNum" type="text" maxlength="6" style="float:left;width:35px;height:32px;font-size:90%;border:1px solid gray;" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)"/>
		                     <font style="float:left;">页</font> 
	                     </span>
	                     <span style="float:left;margin-left:10px;line-height:30px;width:48px;height:32px;font-size:90%;font-weight:500;background:#EEEEEE;border:1px solid gray;cursor:pointer;" onclick="goPage()">确定</span>
	                 </div>
	              </div>  
              </c:if> 
          </div>
