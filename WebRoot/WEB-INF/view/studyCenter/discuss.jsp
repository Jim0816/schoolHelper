  ...
  <head>
    ...
    <script type="text/javascript">
         window.onload = function (){
            var writeDisResult='<%=request.getAttribute("writeDisResult")%>';
            if(writeDisResult=="null"||writeDisResult=="")
				{
				}else{
				   alert(writeDisResult);
		     }
        };
	    ...
	    function checkSubmit(){
	          if(checkUser()==true && checkTitle()==true){
		             //满足提交表单的要求
		             return true;
		        }else{
			         //不满足提交表单的要求
	    ...
	    //检查评论区是否数据为空
	    function checkTitle(){
	       var discussTitle = document.getElementById("discussTitle").value.replace(/\s/g, "");
	       if(discussTitle==""){
	...
	<%
       String submitFlag = UUID.randomUUID().toString();
       session.setAttribute("submitFlag", submitFlag);
       PageUtil discussPage = (PageUtil)session.getAttribute("discussPage");
         ...
         <div class="bg_top_top" >
             ...
             <c:choose>
                 <c:when test="${sessionScope.user==null}">
		             ...
		             <a class="a1" href="${pageContext.request.contextPath }/person/user/login?pageSource=c2" style="float:left;margin-left:10px;font-size:80%;line-height:28px;text-decoration:none;">请登录</a> 
		             <a class="a1" href="${pageContext.request.contextPath }/person/user/regist?pageSource=c2" style="float:left;margin-left:20px;font-size:80%;line-height:28px;text-decoration:none;">[免费注册]</a>
                 </c:when>
                 <c:otherwise>
                     <a class="a1" href="${pageContext.request.contextPath }/person/1/1?pageSource=c2">
                     ...
                     <a class="a1" href="${pageContext.request.contextPath }/person/user/exit?pageSource=c2" style="float:left;margin-left:10px;font-size:80%;line-height:27px;text-decoration:none;">注销</a>
                 </c:otherwise>
             </c:choose>
             ...
             <a class="a1" href="${pageContext.request.contextPath }/person/1/6?pageSource=c2" style="float:right;margin-right:10px;font-size:80%;line-height:28px;text-decoration:none;">卖家中心</a>
             <a class="a1" href="${pageContext.request.contextPath }/person/1/1?pageSource=c2" style="float:right;margin-right:10px;font-size:80%;line-height:28px;text-decoration:none;">个人中心</a>
        </div>
        <div class="bg" onmouseover="javascript:document.getElementById('nevigationBar').style.visibility='hidden';document.getElementById('showBar').style.background='none'">
            <div class="head">
                <span class="center_search">
                     ...
                     <a href="#"><span style="float:left;text-align:center;line-height:35px;width:20%;height:40px;background:#F06F4B;font-size:120%;color:white;" onmouseover="this.style.fontWeight='bold'" onmouseout="this.style.fontWeight='normal'">搜&nbsp贴</span></a>
                </span>
            </div>
            <div class="center">
                ...
                <div class="center_1">
                    <span style="float:left;margin-top:10px;width:100%;height:40px;background:#E3DFDE;border-top:1px solid #F06F4B">
                        ...
                        <font style="float:left;margin-left:10px;margin-top:8px;color:#F06F4B">交流论坛</font>
                    </span>
                ...
                <div id="center_2" class="center_2">
                    ...
                    <div class="center_2_left">
	                    <c:forEach items="${discussPage.getResultList()}" var="item" varStatus="order">
		                    <span style="float:left;width:100%;height:100px;background:white;">
		                        ...
		                        <span style="float:left;margin-left:3%;margin-top:20px;width:75%;height:76px;">
		                            <span style="float:left;text-align:left;width:100%;height:24px;"><a class="aaa" href="${pageContext.request.contextPath }/studyCenter/2/1?disscussId=${item.get('studyDiscussInfoId')}" style="text-decoration:none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">${item.get("discussTitle")}</a></span>
		                            <span style="float:left;margin-top:7px;text-align:left;width:100%;height:38px;font-size:80%;color:gray;overflow:hidden;">${item.get('discussContent')}</span>
		                        </span>
		                        <span style="float:right;margin-right:1%;text-align:right;margin-top:30px;width:15%;height:76px;">
		                            <span style="color:gray;font-size:85%;">${item.get('userName')}</span><br/><br/>
		                            <span style="color:gray;font-size:85%;"><fmt:formatDate value="${item.get('sendDiscussTime')}" pattern="yyyy-MM-dd" /></span>
		                        </span>
		                    </span>
	                    </c:forEach>
                    </div>
                    <div id="center_2_right" class="center_2_right">
                         <c:if test="${user==null}">
                            <div style="float:left;width:100%;height:149px;border-bottom: 1px solid gray;">
	                            ...
	                            <a href="${pageContext.request.contextPath }/person/user/login?pageSource=c2" class="aaa" style="float:left;margin-left:10px;text-decoration:none;font-size:80%;">请登录</a>
                            </div>
                        </c:if>
                        <c:if test="${user!=null}">
                            <div style="float:left;width:100%;height:149px;border-bottom: 1px solid gray;background:white;">
	                            <span style="float:left;margin-top:19px;margin-left:5%;text-align:left;font-size:90%;font-weight:bold;width:95%;height:29px;" >我的帖子</span>
	                            <a href="${pageContext.request.contextPath }/person/1/1?pageSource=c2" title="我的头像"><span style="float:left;margin-left:5%;width:35%;height:60px;border:1px solid gray;padding:5px;" ><img src="${sessionScope.user.getUserPhoto()}" style="float:left;width:100%;height:100%;"/></span></a>
	                            <span style="float:left;width:50%;height:70px;background:white;">
	                                <a href="${pageContext.request.contextPath }/person/1/1?pageSource=c2" class="aaa" style="text-decoration:none;"><span style="float:left;margin-left:5%;text-align:left;width:95%;height:40px;font-size:80%;">${sessionScope.user.getUserName()}</span></a>
	                                <a href="${pageContext.request.contextPath }/person/1/4?pageSource=c2" class="aaa" style="text-decoration:none;font-size:80%;"><span style="float:left;text-align:left;margin-left:5%;width:95%;font-size:80%;">[管理]</span></a>
	                            </span>
                            </div>
                        </c:if>
                ...
                <div class="center_3">
                  <c:if test="${discussPage.getTotalPages()>1}">
                    <div style="float:left;width:83%;height:100%;">
                        <span style="float:left;width:100%;height:50%;color:gray;">
                            <span style="float:left;color:gray;">当前页:${discussPage.getCurrentPage()}/${discussPage.getTotalPages()}</span>
                            <span style="align:center;color:gray;">
                                 <a href="${pageContext.request.contextPath }/studyCenter/2?currentPage=1">首页</a>
				                 
				                 <c:if test="${discussPage.getCurrentPage()==1}">
				                     <a href="javascript:void(0);">上一页</a>
				                 </c:if>
				                 <c:if test="${discussPage.getCurrentPage()>1}">
				                     <a href="${pageContext.request.contextPath }/studyCenter/2?currentPage=${discussPage.getCurrentPage()-1}">上一页</a>
				                 </c:if>
				                 
				                 <c:if test="${discussPage.getCurrentPage()==discussPage.getTotalPages()}">
				                     <a href="javascript:void(0);">下一页</a>
				                 </c:if>
				                 <c:if test="${discussPage.getCurrentPage()<discussPage.getTotalPages()}">
				                     <a href="${pageContext.request.contextPath }/studyCenter/2?currentPage=${discussPage.getCurrentPage()+1}">下一页</a>
				                 </c:if> 
				                 <a href="${pageContext.request.contextPath }/studyCenter/2?currentPage=${discussPage.getTotalPages()}">尾页</a>
                            </span>
                            <span style="float:right;color:gray;">总数:${discussPage.getTotalRows()}</span>
                            <input type="hidden" id="allPageNum" value="${discussPage.getTotalPages()}"/>
                        </span>
             
                        <form action="${pageContext.request.contextPath }/studyCenter/2" method="get" onsubmit="return checkPageNum();">
	                        <span style="float:left;width:100%;height:50%;color:gray;">
	                            <span style="align:center;color:gray;">
	                               &nbsp&nbsp&nbsp&nbsp第<input type="text" id="currentPage" name="currentPage" style="width:20px;height:20px;" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')};"/>
	                                                                                页&nbsp共${discussPage.getTotalPages()}页
                ...
                <div class="center_4">
                <!-- 这里是发帖区域 -->
                    <form action="${pageContext.request.contextPath }/studyCenter/2/3" method="post" onsubmit="return checkSubmit();">
	                    <div style="float:left;width:1000px;height:99%;background:white;border: 1px solid gray;">
	                        <span style="float:left;margin-top:10px;margin-left:2%;width:95%;height:30px;">
	                            ...
	                            <span style="float:left;margin-left:10px;font-size:90%;font-weight:bold;">发表新帖</span>
	                            <span id="tip" style="float:left;margin-left:10px;font-size:90%;font-weight:700;color:white;">标题不能为空</span>
	                        </span>
	                        <span style="float:left;margin-top:10px;margin-left:2%;width:95%;height:40px;">
	                            <input type="text" id="discussTitle" name="discussTitle" style="float:left;width:93%;height:100%;"/>
	                            <img src="./images/studyCenter/discuss/huati.jpg" style="float:right;margin-top:5px;width:55px;height:30px;"/>
	                        </span>
	                        <span style="float:left;margin-top:10px;margin-left:2%;width:95%;height:250px;">
	                            <textarea id="textArea" name="discussContent" rows="11" cols="114" style="float:left;resize:none;"></textarea>
	                        </span>
	                        <input type="hidden" name="writeDiscussId" value="${user.getUserId()}"/>
	                        <input type="hidden" name="discussPhoto" value="www.photo"/>
	                        <input type="hidden" name="submitFlag" value="<%=submitFlag%>"/>
	                        <input type="hidden" name="discussSearchNum" value="0"/>
	                        <input type="submit" value="发帖" title="登录后再发帖" style="float:left;margin-left:2%;width:6%;cursor:pointer;" />
	                    </div>
                   </form>
                </div>   
            </div>
