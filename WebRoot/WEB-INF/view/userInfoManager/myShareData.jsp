    ...
    <div class="bg" onmouseover="javascript:document.getElementById('nevigationBar').style.visibility='hidden';document.getElementById('showBar').style.background='none'">
      ...
      <div class="middle">
          <div class="middle_left">
              ...
              <span class="middle_left_btn"><a href="javascript:void(0)" style="float:left;text-align:left;text-decoration:none;font-size:85%;">我的分享</a></span>
          ...
          <div class="middle_right">
                      <div class="middle_right_1">
		                  <font style="float:left;"> 我的分享 </font>
		              </div>
		              <c:choose>
			              <c:when test="${sessionScope.myShareData.getTotalRows()>0}">
				              <div class="middle_right_2">
						           <c:choose>
						               <c:when test="${sessionScope.myShareData.getTotalPages()==sessionScope.myShareData.getCurrentPage() || sessionScope.myShareData.getTotalPages()==0}">
						               ...
						               <c:otherwise>
						                   <a href="${pageContext.request.contextPath }/person/1/5?page=${sessionScope.myShareData.getCurrentPage()+1}" title="下一页"><span class="pageSelect_next" onmouseover="this.style.background='gray'" onmouseout="this.style.background='white'">&gt</span></a>
						               </c:otherwise>
						           </c:choose>
						           
						           <c:choose>
						               <c:when test="${sessionScope.myShareData.getCurrentPage()>1}">
						                   <a href="${pageContext.request.contextPath }/person/1/5?page=${sessionScope.myShareData.getCurrentPage()-1}" title="上一页"><span class="pageSelect_pre" onmouseover="this.style.background='gray'" onmouseout="this.style.background='white'">&lt</span></a>
						               </c:when>
						               <c:otherwise>
						           ...
						           <c:if test="${sessionScope.myShareData.getTotalPages()>0}">
						               <span style="float:right;margin-right:20px;text-align:right;font-size:85%;line-height:30px;">当前   ${sessionScope.myShareData.getCurrentPage()}/${sessionScope.myShareData.getTotalPages()}</span>
						           </c:if>
						           <span style="float:right;margin-right:20px;text-align:right;font-size:85%;line-height:30px;">共有${sessionScope.myShareData.getTotalRows()}条帖子</span>
						      </div>
				              ...
				              <c:forEach items="${sessionScope.myShareData.getResultList()}" var="item" varStatus="idxStatus">
				                  <div class="middle_right_4" onmouseover="this.style.background='#EEEEEE'" onmouseout="this.style.background='white'">
					                  <span style="float:left;margin-top:10px;margin-left:2%;width:68%;height:140px;">
					                      <a href="${pageContext.request.contextPath }/studyCenter/3/1?dataId=${item.get('dataShareInfoId')}"><span style="float:left;width:100%;height:30px;text-align:left;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;background:#EEEEEE;">《${item.get('dataTitle')}》</span></a>
					                      <span style="float:left;width:100%;height:110px;text-align:left;font-size:85%;color:gray;overflow:hidden;">
					                          <span style="float:left;margin-left:2%;width:98%;height:30px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;">学科类型:&nbsp&nbsp&nbsp${item.get('dataTypeName')}</span>
					                          <span style="float:left;margin-left:2%;width:98%;height:30px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;">分享时间:&nbsp&nbsp&nbsp${item.get('shareDate')}</span>
					                          <span style="float:left;margin-left:2%;width:98%;height:30px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;">下载次数:&nbsp&nbsp&nbsp${item.get('loadNum')}</span>
					                      </span>
					                  </span>
					                  <span style="float:left;margin-top:10px;margin-left:2%;width:26%;height:140px;">
					                      <a href="${pageContext.request.contextPath }/person/2/2?d=${item.get('dataShareInfoId')}" style="line-height:130px;">删除</a>
					                  </span>
					              </div>
				              </c:forEach>
			              </c:when>
